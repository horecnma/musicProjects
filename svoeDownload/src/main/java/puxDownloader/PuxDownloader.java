...
public class PuxDownloader
{
	...
	void download(File exampleFile) throws IOException
	{
		Document result = loadFindPage(FilenameUtils.getBaseName(exampleFile.getPath()));
		int exampleFileDuration = getDuration(exampleFile);
		List<PuxSong> songs = result.select("#list_of_songs")
						...
						.filter(song -> Math.abs(song.getDuration() - exampleFileDuration) < 10)
...
//		downloadSongs(songs);
	...
	private void downloadSongs(List<PuxSong> songs) throws IOException
	{
	...
	private void downloadSong(PuxSong puxSong, int i) throws IOException
	{
	...
	private Document loadFindPage(String exampleFileName) throws IOException
	{
	...
	private void addParameter(HttpURLConnection con, String s) throws IOException
	{
...
//		}
	...
	public HttpURLConnection openConnection(String url, String methodType) throws IOException
	{
		URL obj = new URL(url);
